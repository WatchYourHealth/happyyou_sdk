...
public class SearchListAdapter extends RecyclerView.Adapter<SearchListAdapter.ViewHolder> {
    ...
    private List<StateModel> filteredList;
    ...
    public SearchListAdapter(List<StateModel> list, OnItemClickListener listener) {
    ...
    public void onBindViewHolder(@NonNull SearchListAdapter.ViewHolder holder, int position) {
        final String item = filteredList.get(position).getStateName();
    ...
    public void filter(String query) {
        ...
        if (query == null || query.trim().isEmpty()) {
            ...
            for (StateModel item : originalList) {
                if (item.getStateName().toLowerCase().contains(query.toLowerCase())) {
