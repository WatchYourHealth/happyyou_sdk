...
public class AllBlogsAdapter extends RecyclerView.Adapter<AllBlogsAdapter.ViewHolder> {
    ...
    private final AllBlogsAdapter.OnItemClickListener listener;

    public AllBlogsAdapter(Context context, List<GetQuickReadResponse.Data.AllBlogs> allBlogsList, OnItemClickListener listener) {
    ...
    @NonNull
    @Override
    public AllBlogsAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
    ...
    @Override
    public void onBindViewHolder(@NonNull AllBlogsAdapter.ViewHolder holder, int position) {
        switch (position) {
            case 0:
                for (int i = 0; i < 2; i++) {
                    final int index = i;
                    if (i == 0) {
                        if (!allBlogsList.isEmpty()) {
                            holder.binding.tvArticle1.setText(allBlogsList.get(i).getArticleName());
                            Glide.with(context)
                                    .load(allBlogsList.get(i).getImgPath())
...
//                .placeholder(circularProgressDrawable)
                            ...
                            holder.binding.cvArticle1.setOnClickListener(view -> {
                                ...
                                intent.putExtra("article_code", allBlogsList.get(index).getArticleCode());
                                context.startActivity(intent);
                            });

                            if(allBlogsList.get(i).getIsBookMarked() == 1){
                            ...
                            holder.binding.ivBookmark1.setOnClickListener(view -> listener.onClick(allBlogsList.get(index).getArticleCode(), allBlogsList.get(index).getIsBookMarked() != 1));
                        }
                    }
                    else {
                        if (allBlogsList.size() > 1) {
                            holder.binding.tvArticle2.setText(allBlogsList.get(i).getArticleName());
                            Glide.with(context)
                                    .load(allBlogsList.get(i).getImgPath())
                                    .error(R.drawable.dummy_image)
//                .placeholder(circularProgressDrawable)
                            ...
                            holder.binding.ivBookmark2.setOnClickListener(view -> listener.onClick(allBlogsList.get(index).getArticleCode(), allBlogsList.get(index).getIsBookMarked() != 1));
                        } else {
                            holder.binding.rlArticle2Hide.setVisibility(View.GONE);
                        }
                    }

                    if (allBlogsList.size() < 2) {
                        holder.binding.cvArticle2.setVisibility(View.GONE);
                    } else if (allBlogsList.size() < 3) {
...
//                .placeholder(circularProgressDrawable)
                            ...
                            holder.binding.cvArticle1.setOnClickListener(view -> {
                                Intent intent = new Intent(context, QuickReadActivity.class);
                                intent.putExtra("article_code", allBlogsList.get(index).getArticleCode());
//                                intent.putExtra("title", allBlogsList.get(index).getArticleName());
//                                intent.putExtra("description", allBlogsList.get(index).getArticleName());
                                //intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                context.startActivity(intent);
                            });
                            if(allBlogsList.get(i).getIsBookMarked() == 1){
...
//                .placeholder(circularProgressDrawable)
    ...
    public int getItemCount() {
        if(allBlogsList.size() > 6){
            return 6;
        }else if (allBlogsList.size() > 2) {
            return (int) Math.ceil(Double.parseDouble(String.valueOf(allBlogsList.size())) / 2.0);
        } else {
            return 1;
        }
    }
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull AbsorbMostReadAdapterBinding binding) {
