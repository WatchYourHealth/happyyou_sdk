...
public class ActometerStackBarHappyInsightsTagsAdapter extends RecyclerView.Adapter<ActometerStackBarHappyInsightsTagsAdapter.MyViewHolder> {

    Context context;
    List<String> tagNameList;
    private int selectedItem = 0;
    private int lastSelected = 0;
    String diagnosticType, diagnosticPeriod;
    int communityId;

    public ActometerStackBarHappyInsightsTagsAdapter(Context context, List<String> tagNameList, int selectedItem, int communityId, String diagnosticPeriod, String diagnosticType) {
    ...
    public void updateDiagnosticPeriod(String diagnosticPeriod) {
        this.diagnosticPeriod = diagnosticPeriod;
    }

    public void updateDiagnosticType(String diagnosticType) {
        this.diagnosticType = diagnosticType;
    }
    ...
    public void onBindViewHolder(@NonNull MyViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        holder.binding.tvArticleTag.setOnClickListener(view -> {
            String tagName = "";
            if (context instanceof ActoMeterCommunityActivity) {
                switch (tagNameList.get(position)) {
                    case "H20":
                        tagName = WATER;
                        break;
                    case "Pillow Time":
                        tagName = SLEEP;
                        break;
                    case "Happy Footprints":
                        tagName = STEPS;
                        break;
                    case "Cal - Count":
                        tagName = CONSUMEDGRAPH;
                        break;
                    case "Zen Zone":
                        tagName = MEDITATION;
                        break;
                ...
                {*/
                ((ActoMeterCommunityActivity) context).fetchCommunityGraphHappyInsightsStack(DAILY, tagName, communityId, diagnosticPeriod, diagnosticType);
                /*}
                else
                {
                    ((ActOMeterActivity) context).fetchGraphData(DAILY, tagName,communityId,DAILY,"Heart rate");
                }*/
            }
